...
Repository-wide coding standards are documented in
[`.github/copilot-instructions.md`](.github/copilot-instructions.md), with
technology-specific guidance in the [instruction files](.github/instructions/).
In particular:

- Comments explain intent and non-obvious decisions rather than restating code.
- Exported functions in `db/` and `src/lib/` use TSDoc/JSDoc for their purpose,
  parameters, and return values.
- Reusable Astro components document their `Props` interfaces and public
  contracts.
- TypeScript uses single quotes, semicolons, four-space indentation, and
  trailing commas in multiline structures; ESLint checks the baseline formatting
  rules.
